...
class GIS:
    ...
    def generate_token(self):
        ...
        if succes :
            ...
            login_data = urllib.parse.urlencode({'user[email]': 'mohammed.hammad@aiesec.net', 'user[password]': 'Meraki77','authenticity_token':auth_token,'commit':'Sign in'}).encode()
            ...
            for cookie in self.cj:
                if cookie.name == 'expa_token':
                    token = cookie.value
            if token is None:
                raise Exception('Unable to generate a token for {0}!'.format('mohammed.hammad@aiesec.net'))
            return token
        else:
